...
export function AshaAuthModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await verifyAndLogin(workerId, pin, "asha");
    if (!res.success) {
  ...
  const handleSelectRegisteredWorker = async (id: string, codePin: string) => {
    setWorkerId(id);
    setPin(codePin);
    setError(null);
    const res = await verifyAndLogin(id, codePin, "asha");
    if (res.success) {
      onSuccess?.();
      onClose();
    }
          ...
          <div className="space-y-1.5">
            ...
            >
              <div>
                ...
                <span className="text-on-surface-variant ml-1.5">(ASHA-WB-401 • Shibpur Village)</span>
              </div>
            ...
            >
              <div>
                ...
                <span className="text-on-surface-variant ml-1.5">(ASHA-WB-402 • Maharajganj)</span>
              </div>
