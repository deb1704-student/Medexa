...
export function BlockOfficeAuthModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await verifyAndLogin(officerId, pin, "block_officer");
    if (!res.success) {
  ...
  const handleSelectRegisteredOfficer = async (id: string, codePin: string) => {
    setOfficerId(id);
    setPin(codePin);
    setError(null);
    const res = await verifyAndLogin(id, codePin, "block_officer");
    if (res.success) {
      onSuccess?.();
      onClose();
    }
          ...
          <div className="space-y-1.5">
            ...
            >
              <div>
                ...
                <span className="text-on-surface-variant ml-1.5">(BHO-WB-204 • Dwariknagar Rural Hospital)</span>
              </div>
            ...
            >
              <div>
                ...
                <span className="text-on-surface-variant ml-1.5">(BHO-WB-205 • Namkhana CHC)</span>
              </div>
